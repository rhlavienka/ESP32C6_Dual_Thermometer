...
This guide explains how to flash pre-compiled firmware to your ESP32-H2 without installing ESP-IDF framework.

## Download Firmware

Download the latest firmware from [GitHub Releases](https://github.com/rhlavienka/ESP32H2_Dual_Thermometer/releases/latest).

**Direct download links (v1.1.0):**
- [bootloader.bin](https://github.com/rhlavienka/ESP32H2_Dual_Thermometer/releases/download/v1.1.0/bootloader.bin)
- [partition-table.bin](https://github.com/rhlavienka/ESP32H2_Dual_Thermometer/releases/download/v1.1.0/partition-table.bin)
- [ESP32H2_zigbee_thermometer.bin](https://github.com/rhlavienka/ESP32H2_Dual_Thermometer/releases/download/v1.1.0/ESP32H2_zigbee_thermometer.bin)
- [flasher_args.json](https://github.com/rhlavienka/ESP32H2_Dual_Thermometer/releases/download/v1.1.0/flasher_args.json)
- [checksums.txt](https://github.com/rhlavienka/ESP32H2_Dual_Thermometer/releases/download/v1.1.0/checksums.txt) - SHA256 checksums for verification

You will need these files:
- `bootloader.bin` - Bootloader
- `partition-table.bin` - Partition table
- `ESP32H2_zigbee_thermometer.bin` - Main application
...
1. Connect ESP32-H2 to your computer via USB-C
2. Open terminal/command prompt in the folder with firmware files
3. Run:

**Windows:**
```cmd
esptool.py --chip ESP32H2 write_flash @flasher_args.json
```
...
```cmd
esptool.py --port COM3 --chip ESP32H2 write_flash @flasher_args.json
```
...
```cmd
esptool.py --chip ESP32H2 --baud 921600 write_flash ^
  0x0 bootloader.bin ^
  0x8000 partition-table.bin ^
  0x10000 ESP32H2_zigbee_thermometer.bin
```
...
1. Run the tool and select **ESP32-H2**
2. Add files with addresses:
   - `bootloader.bin` → `0x0`
   - `partition-table.bin` → `0x8000`
   - `ESP32H2_zigbee_thermometer.bin` → `0x10000`
...
```cmd
esptool.py --chip ESP32H2 erase_flash
```
Then flash again.
...
- Flash size: 4MB (default for Seeed Studio ESP32-H2 Super Mini)
- Flash mode: DIO
...
See [FAQ.md](FAQ.md) for common questions or open an issue on [GitHub](https://github.com/rhlavienka/ESP32H2_Dual_Thermometer/issues).
