# Zigbee2MQTT Configuration Guide for ESP32-H2 Dual Thermometer + Contact Sensor

Complete guide for adding support for the ESP32-H2 dual temperature + contact sensor device to Zigbee2MQTT.

## Overview

This ESP32-H2 device implements a Zigbee Router with two DS18B20 temperature sensors and one contact/door sensor on separate endpoints:
- **Endpoint 11**: Temperature sensor 1 (DS18B20 #1) + OTA
- **Endpoint 12**: Temperature sensor 2 (DS18B20 #2)
- **Endpoint 13**: Contact/door sensor (IAS Zone - Contact Switch)
- **Zigbee Model**: `ESP32H2.TH`
...
1. **Disconnect** ESP32-H2 from power
...
1. Simply power on or reset the ESP32-H2
...
Zigbee2MQTT:warn  Device '0x00124b001234abcd' with Zigbee model 'ESP32H2.TH' and manufacturer name 'Espressif' is NOT supported
...
3. Create file `ESP32H2_thermometer.js` with this content:

```javascript
/**
 * Zigbee2MQTT External Converter for ESP32-H2 Dual Thermometer
 * 
 * Device: ESP32-H2 with dual DS18B20 temperature sensors
...
const definition = {
    ...
    configure: async (device, coordinatorEndpoint, logger) => {
        ...
        logger.info('ESP32H2 Dual Thermometer configured successfully');
    },
};
...
7. Create `ESP32H2_thermometer.js` with the code above
...
2. Go to **Devices** tab
3. Find your ESP32-H2 device (should now show as `ESP32H2-DUAL-TEMP`)
...
2. Set **Friendly name**: `ESP32H2_thermometer` (or your preferred name)
3. Save changes
...
- Via MQTT:
  ```bash
  mosquitto_sub -h localhost -t 'zigbee2mqtt/ESP32H2_thermometer' -v
  ```
...
- `sensor.ESP32H2_thermometer_temperature_sensor1`
- `sensor.ESP32H2_thermometer_temperature_sensor2`

### 4.2 Customize Entity Names

**Via Home Assistant UI:**
1. Go to **Settings** → **Devices & Services** → **Entities**
2. Search for `ESP32H2_thermometer`
...
entities:
  - entity: sensor.ESP32H2_thermometer_temperature_sensor1
    name: Living Room
    icon: mdi:thermometer
  - entity: sensor.ESP32H2_thermometer_temperature_sensor2
    name: Kitchen
    icon: mdi:thermometer
```
...
Modify the `configure` function in `ESP32H2_thermometer.js`:
...
sensor:
  - platform: mqtt
    name: "ESP32H2 Link Quality"
    state_topic: "zigbee2mqtt/ESP32H2_thermometer"
...
[
    {
        "id": "mqtt-in",
        "type": "mqtt in",
        "topic": "zigbee2mqtt/ESP32H2_thermometer",
        "broker": "mqtt-broker",
        "outputs": 1
    },
    {
...
1. **Device Type**: ESP32-H2 functions as a **Zigbee Router**, not an end device
...
4. **IEEE Address**: Auto-generated from ESP32-H2 MAC address (unique per device)
...
- **ESP32-H2 Documentation**: https://www.espressif.com/en/products/socs/ESP32-H2
