...
Simple examples for using the ESP32-H2 Dual Thermometer in Home Assistant.
  ...
  - alias: "High Temperature Alert"
    ...
    trigger:
      - platform: numeric_state
        entity_id: sensor.ESP32H2_thermometer_temperature_sensor1
        above: 28
        for:
          minutes: 5
    action:
      - service: notify.mobile_app
        data:
          ...
          message: >
            Living room temperature is {{ states('sensor.ESP32H2_thermometer_temperature_sensor1') }}°C
```
...
template:
  - sensor:
      - name: "Temperature Difference"
        ...
        state: >
          {% set t1 = states('sensor.ESP32H2_thermometer_temperature_sensor1') | float(0) %}
          {% set t2 = states('sensor.ESP32H2_thermometer_temperature_sensor2') | float(0) %}
          {{ (t1 - t2) | round(1) }}
